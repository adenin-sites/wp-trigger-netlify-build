...
/**
 * Plugin Name: WP Trigger Netlify Build - Charlotte edit
...
{
  add_options_page('WP Trigger Netlify Build', 'Netlify Build', 'manage_options', 'wp-trigger-netlify-build', 'wp_trigger_netlify_build_options_page', netlify_icon());
}
...
function wp_trigger_netlify_build_settings_init()
{
  ...
  add_settings_section(
    'wp_trigger_netlify_master_build_section',
    __('Master', 'wordpress'),
    '',
    'wp_trigger_netlify_build'
  );
  ...
  add_settings_field(
    'wp_trigger_netlify_master_build_webhook_url',
    __('Netlify Webhook URL', 'wordpress'),
    'wp_trigger_netlify_master_build_webhook_url_render',
    'wp_trigger_netlify_build',
    'wp_trigger_netlify_master_build_section'
  );
  add_settings_field(
    'wp_trigger_netlify_master_build_status_image',
    __('Netlify Status Image', 'wordpress'),
    'wp_trigger_netlify_master_build_status_image_render',
    'wp_trigger_netlify_build',
    'wp_trigger_netlify_master_build_section'
  );
  add_settings_field(
    'wp_trigger_netlify_master_build_status_link',
    __('Netlify Status Link', 'wordpress'),
    'wp_trigger_netlify_master_build_status_link_render',
    'wp_trigger_netlify_build',
    'wp_trigger_netlify_master_build_section'
  );
}
...
// Master
function wp_trigger_netlify_master_build_webhook_url_render()
{
  $options = get_option('wp_trigger_netlify_build_settings');
?>
  <input type='text' name='wp_trigger_netlify_build_settings[wp_trigger_netlify_master_build_webhook_url]' value='<?php echo $options['wp_trigger_netlify_master_build_webhook_url']; ?>' class="regular-text">
  <span class="description"><?php esc_attr_e('The URL provided by Netlify for a custom webhook', 'WpAdminStyle'); ?></span><br>
<?php
}

function wp_trigger_netlify_master_build_status_image_render()
{
  $options = get_option('wp_trigger_netlify_build_settings');
?>
  <input type='text' name='wp_trigger_netlify_build_settings[wp_trigger_netlify_master_build_status_image]' value='<?php echo $options['wp_trigger_netlify_master_build_status_image']; ?>' class="regular-text">
  <span class="description"><?php esc_attr_e('The URL provided by Netlify for the image that shows site status', 'WpAdminStyle'); ?></span><br>
<?php
}

function wp_trigger_netlify_master_build_status_link_render()
{
  $options = get_option('wp_trigger_netlify_build_settings');
?>
  <input type='text' name='wp_trigger_netlify_build_settings[wp_trigger_netlify_master_build_status_link]' value='<?php echo $options['wp_trigger_netlify_master_build_status_link']; ?>' class="regular-text">
  <span class="description"><?php esc_attr_e('The URL provided by Netlify for the link to the site status', 'WpAdminStyle'); ?></span><br>
<?php
}
...
function wordpress_netlify_enqueue($hook)
{
  $options = get_option('wp_trigger_netlify_build_settings');
  $data_array = array(
    'stageurl' => $options['wp_trigger_netlify_build_webhook_url'],
    'masterurl' => $options['wp_trigger_netlify_master_build_webhook_url']
  );
  ...
  wp_localize_script('wp-trigger-netlify-build', 'wpTriggerNetlifyBuildVars', $data_array);
}
...
{
  echo "<table><tbody><tr>";
  echo "<td style='padding-right: 36px'><h3>Stage</h3>";
  $options = get_option('wp_trigger_netlify_build_settings');
  ...
  $markup .= '</a></td>';
  echo $markup;
  echo "<td><h3>Master</h3>";
  $markupMaster .= '<a href="' . $options['wp_trigger_netlify_master_build_status_link'] . '" target="_blank" rel="noopener noreferrer">';
  $markupMaster .= '<img src="' . $options['wp_trigger_netlify_master_build_status_image'] . '" alt="Netlify Status" />';
  $markupMaster .= '</a></td>';
  echo $markupMaster;
  echo "</tr></tbody></table>";
}
